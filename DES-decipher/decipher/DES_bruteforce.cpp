...
int main()
{
    // int sw1;
    int diff=0;
    BYTE key_from_file[56], key_duplicate[56];
    int rounds;
    BYTE cipher_predicted[9];
    char flag;
    int i;
    string str_key;
    ifstream fi;
    fi.open("possible_keys.txt");
    while( fi>>str_key ){
        for(i=0;i<56;i++){
            key_from_file[i]=str_key[i]-48;
        	key_duplicate[i]=key_from_file[i];
        }
    	
    	rounds=6;
    	//fghifghifghifghi --> plaintext
    	BYTE plain[9]={1,35, 1,35,1,35, 1,35,'\0'};
    	//pkqojmmkopjlfluu --> ciphertext
    	BYTE cipher_original[9]={165,185,71,117,154,70,6,255,'\0'};
    	flag='N';
    	diff=0;
    	set_the_key(0,key_from_file,rounds);
    	des(plain,cipher_predicted,rounds,flag);
  	    for(i=0;(i<8 && diff==0);i++){
        	if(int(cipher_original[i])!=int(cipher_predicted[i]))
            	diff=1;
    	}
    	if(diff==0){
	        for(i=0;i<56;i++){
	            cout<<int(key_duplicate[i]);
        	}
          cout<<"\n";
    	}
